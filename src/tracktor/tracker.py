...
#         print('pos: ',pos)
        # regress
        boxes, scores = self.obj_detect.predict_boxes(pos) # FIX THIS, can I just replace with detect, don't think so
    ...
    def reid(self, blob, new_det_pos, new_det_scores):
        ...
        if self.do_reid:
            new_det_features = self.reid_network.test_rois(
                blob['img'].to('cpu'), new_det_pos).data
    ...
    def get_appearances(self, blob):
        ...
        new_features = self.reid_network.test_rois(np.array(blob['img'].cpu().numpy(),dtype=np.uint8), self.get_pos()).data
        return new_features
    ...
    def step(self, blob):
        ...
        self.obj_detect.load_image(blob['img']) # FIX THIS


        boxes, scores = self.obj_detect.detect(blob['img']) # FIX THIS
        ...
        if len(self.tracks):
            ...
            if len(self.tracks):
                ...
                if keep.nelement() > 0 and self.do_reid:
                        new_features = self.get_appearances(blob)   # failing here 
